...
async function findEpisodesExercises(client) {
  ...
  const result1 = await client.db("databaseWeek3").collection("bob_ross_episodes").findOne({
    title: "BLACK RIVER"
  });
  console.log(
  ...
  const result2 = await client.db("databaseWeek3").collection("bob_ross_episodes").find({ elements: { $elemMatch: { $regex: /CLIFF/i } } });
  const result2Array = await result2.toArray();
  const episodeTitles = result2Array.map((episode) => episode.title);
  console.log(
    `The episodes that Bob Ross painted a CLIFF are ${episodeTitles}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]
  const result3 = await client.db("databaseWeek3").collection("bob_ross_episodes").find({
    $and: [
      { elements: { $elemMatch: { $regex: /CLIFF/i } } },
      { elements: { $elemMatch: { $regex: /LIGHTHOUSE/i } } },
    ],
  });
  const result3Array = await result3.toArray();
  const episodeTitles2 = result3Array.map((episode) => episode.title);
  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${episodeTitles2}`
  );
}
...
async function updateEpisodeExercises(client) {
  ...
  const result1 = await client.db("databaseWeek3").collection("bob_ross_episodes").updateMany(
    { elements: { $in: ["BUSHES"] } },
    { $set: { "elements.$": "BUSH" } }
  );
